...
/*
    cout<<"Match extracted"<<endl;
    cout<<match.OFB_TAG<<endl;
    cout<<match.OFB_ETH_SRC<<endl;
    cout<<match.OFB_ETH_DST<<endl;
    cout<<match.OFB_IN_PORT<<endl;
    cout<<match.OFB_ETH_TYPE<<endl;
*/
...
//        cout<< "at"<<mac<<".........Lookup for message from stream "<<qframe->getStreamID()<<", tag:"<<qframe->getmatchVersion()<<" at t:"<<simTime()<<endl;
    ...
    if (lookup != nullptr) {
        ...
        //cout << "Found entry in flow table." << '\n';
       ...
       // cout << "No Entry Found contacting controller" << '\n';
...
oxm_basic_match OF_RelayUnit::extractMatch(EthernetIIFrame* frame) {
    ...
    if (CoRE4INET::EthernetIIFrameWithQTag *qframe = dynamic_cast<CoRE4INET::EthernetIIFrameWithQTag*>(frame)){
        match.OFB_VLAN_VID = qframe->getVID();
        match.OFB_TAG = qframe->getmatchVersion();  //rule version tag
    }
    else
        match.OFB_TAG = 0;
    ...
    //IInterfaceTable *inet_ift = getModuleFromPar<IInterfaceTable>(par("interfaceTableModule"), this);
    //MACAddress mac = inet_ift->getInterface(0)->getMacAddress();
    //cout<< "at"<<mac<<" message is tag is read as "<<match.OFB_TAG<<" by the OF relay"<<simTime()<<endl;
